    ...
    for (int p=0;  p<npatches;  p++)    { // For each patch
        for (int i=0;  i<=n; i++) {       // Grid in u direction
            ...
            for (int j=0;  j<=n; j++) { // Grid if v direction
                ...
                Pnt.push_back(vec4(V[0], V[1], V[2], 1.0f));
...
Sphere::Sphere(const int n)
{
    ...
    for (int i=0;  i<=n*2;  i++) {
        ...
        for (int j=0;  j<=n;  j++) {
            ...
            Tan.push_back(vec3(-sin(s), cos(s), 0.0f));
            if (i>0 && j>0) {
...
{
    diffuseColor = vec3(0.8f, 0.8f, 0.5f);
    specularColor = vec3(1.0f, 1.0f, 1.0f);
    shininess = 120.0f;
...
int Ply::vertex_cb(p_ply_argument argument) {
    ...
    staticPnt[index] = (float)c;
    if (index==2) {
        staticPnt[3] = 1.0;
        ply->Pnt.push_back(staticPnt);
        ply->Tan.push_back(vec3()); }
    return 1;
}
...
void ComputeTangent(Ply* ply)
{
    ...
    int j = ply->Tri[t][1];
    int k = ply->Tri[t][2];
    vec2 A = ply->Tex[i] - ply->Tex[k]; 
    vec2 B = ply->Tex[j] - ply->Tex[k];
    float d = A[0]*B[1] - A[1]*B[0];
    float a =  B[1]/d;
    float b = -A[1]/d;
    vec4 Tan = a*ply->Pnt[i] + b*ply->Pnt[j] + (1.0f-a-b)*ply->Pnt[k];
    ply->Tan[i] = ply->Tan[j] = ply->Tan[k] = normalize(Tan.xyz());
     ...
     low(_low), high(_high)
{
    ...
    for (int i=0;  i<=n;  i++) {
        ...
        for (int j=0;  j<=n;  j++) {
            ...
            Pnt.push_back(vec4(x, y, z, 1.0f));
            vec3 du(1.0f, 0.0f, (zu-z)/h);
            vec3 dv(0.0f, 1.0f, (zv-z)/h);
...
Quad::Quad(const int n)
{
    ...
    diffuseColor = vec3(0.3f, 0.2f, 0.1f);
    specularColor = vec3(1.0f, 1.0f, 1.0f);
    shininess = 120.0f;
    ...
    for (int i=0;  i<=n;  i++) {
        ...
        for (int j=0;  j<=n;  j++) {
            float t = j/float(n);
            Pnt.push_back(vec4(s*2.0f*r-r, t*2.0f*r-r, 0.0f, 1.0f));
            Nrm.push_back(vec3(0.0f, 0.0f, 1.0f));
            Tex.push_back(vec2(s, t));
            Tan.push_back(vec3(1.0f, 0.0f, 0.0f));
            if (i>0 && j>0) {
