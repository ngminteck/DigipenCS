...
BehaviorNode::BehaviorNode() : agent(nullptr), status(NodeStatus::READY), result(NodeResult::IN_PROGRESS), parent(nullptr), weight{ 0 }
{}
...
unsigned int BehaviorNode::get_weight() const
{
	return weight;
}

void BehaviorNode::set_weight(unsigned int weight)
{
	weight = weight;
}