...
{
	Matrix x; x.setToRotation(v.x(), Vector3(1, 0, 0));
	Matrix y; y.setToRotation(v.y(), Vector3(0, 1, 0));
	Matrix z; z.setToRotation(v.z(), Vector3(0, 0, 1));
	*this = (x * y * z);
}
...
Vector3 axisRotToEuler(float radian, Vector3 axis)
{
    ...
    if ((x * y * t + z * s) > 0.998) { // north pole singularity detected
        heading = 2 * atan2(x * sin(radian / 2), cos(radian / 2));
        attitude = M_PI / 2;
        bank = 0;
...
Quaternion Slerp(Quaternion q1, Quaternion q2, float t)
{
    ...
    // for Quaternion Slerp we have to ceil it to the acos range because precision messes up in some cases
    ...
    if (cosom < 0.9999f)
    {
        float theta = acos(cosom);
        float deno = 1.0f / sin(theta);
        coeff1 = sin((1.0f - t) * theta) * deno;
        coeff2 = sin(t * theta) * deno;
    }
    else {
        coeff1 = 1.0f - t;
        coeff2 = t;
