...
public:
   ...
   float length() const { return sqrtf( lengthSquared() ); }
	 ...
	 static float Dot(Vector3 p1, Vector3 p2) { return (p1.x() * p2.x()) + (p1.y() * p2.y()) + (p1.z() * p2.z()); }
...
public:
   ...
   void setToRotation(
	 ...
	 void setToRotation( const Vector3& v );
   ...
   // Create a vector for the axis-angle represenation of this matrix, as described here:
   // https://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToAngle/index.htm
   // the axis is returned in x/y/z, and the rotation in w (in radians).
   // Requires a matrix with rotations, only, no translations or scalings.
   Vector3 calculateAxisAngle();
};
...
private:
   float _p[4];
public:
   Quaternion() { _p[0] = _p[1] = _p[2] = _p[3] = 0; }
   ...
   Quaternion& operator=( const Quaternion& p ) {
	 ...
	 static float Dot(Quaternion q1, Quaternion q2){ return q1.x() * q2.x() + q1.y() * q2.y() + q1.z() * q2.z() + q1.w() * q2.w(); }
