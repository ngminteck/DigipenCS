...
const float Camera::_initialFieldOfViewSemiAngle = PI_F * 15 / 180.0f;
...
void Camera::computeViewportWidthAndHeight() {
   ...
   if ( viewport_width_in_pixels < viewport_height_in_pixels ) {
      _viewport_width = 2.0f * _viewport_radius;
      ...
      _viewport_height = 2.0f * _viewport_radius;
...
void Camera::reset() {
   ...
   float tangent = tanf( _initialFieldOfViewSemiAngle );
...
void Camera::resizeViewport(
   ...
   _viewport_x2 = viewport_x2;
   _viewport_y2 = viewport_y2;
   _viewport_cx = ( _viewport_x1 + _viewport_x2 ) * 0.5f;
   _viewport_cy = ( _viewport_y1 + _viewport_y2 ) * 0.5f;
...
void Camera::zoomIn( float delta_pixels ) {
   ...
   // this should be a little greater than 1.0
   static const float magnificationFactorPerPixel = 1.005f;

   _viewport_radius *= powf( magnificationFactorPerPixel, - delta_pixels );
...
void Camera::orbit(
   ...
   float radiansPerPixel = 1.0f / pixelsPerDegree * PI_F / 180;
...
void Camera::pitchCameraUp( float delta_pixels ) {
   ...
   float angle = delta_pixels / pixelsPerDegree * PI_F / 180;
...
void Camera::zBufferLookAt( int pixel_x, int pixel_y ) {
   ...
   if ( z < 1 ) {
      ...
      Matrix M2;
      M2.setToFrustum(
         - 0.5f * _viewport_width,  0.5f * _viewport_width,    // left, right
         - 0.5f * _viewport_height, 0.5f * _viewport_height,   // bottom, top
         _near_plane, _far_plane,
         true
      );
