   ...
   if ( ! isCool ) {
      ...
      glBegin( GL_LINES );
         for ( int dim = 0; dim < 3; ++dim ) {
            Vector3 v = ( box.getCorner(1<<dim) - box.getCorner(0) )*0.1f;
            for ( int a = 0; a < 2; ++a ) {
               for ( int b = 0; b < 2; ++b ) {
...
void drawCircle(
   ...
   glPushMatrix();
      ...
      int nbSides = (int)( 2 * PI_F * radius / arcLengthPerPixel + 1 );

      float deltaAngle = 2 * PI_F / nbSides;
      float angle;
      if ( isDashed ) {
         glBegin( GL_LINES );
            int i = 0;
...
void drawShadedCircle(
   ...
   glPushMatrix();
      ...
      if ( isArc ) {
         ASSERT( arcAngle >= 0 );
         i_max = ROUND( i_max * arcAngle / (2 * PI_F) );
      ...
      for ( int i = 1; i <= i_max; ++i ) {
         ...
         newPoint = Point3( radius * sinf(angle), radius * cosf(angle), 0 );
...
void drawCone(
   ...
   glPushMatrix();
      glMultMatrixf( m.get() );
      float h = lateralLength * cosf( semiAngle );
      float r = lateralLength * sinf( semiAngle );
      Point3 * basePoints = new Point3[ numLateralSides ];
      float deltaAngle = 2*PI_F / numLateralSides;
      int i;
      glBegin( GL_LINES );
         for ( i = 0; i < numLateralSides; ++i ) {
            float angle = i * deltaAngle;
            basePoints[i] = Point3( r*cosf(angle), r*sinf(angle), -h );
            glVertex3f( 0, 0, 0 );
...
void drawFrame(
   ...
   glEnd();
   if ( drawArrowHeads ) {
      drawCone( C, -k, PI_F/8, length/7, 3, false );
      if ( isColoured ) glColor3fv( j_colour.get() );
      drawCone( B, -j, PI_F/8, length/7, 3, false );
      if ( isColoured ) glColor3fv( i_colour.get() );
      drawCone( A, -i, PI_F/8, length/7, 3, false );
   }
}
...
void drawRay( const Ray& ray, float length, float arrowHeadLength ) {
   ...
   glEnd();
   drawCone( p+v*length, -v, PI_F/8, v.length()*arrowHeadLength, 5, false );
}
...
void drawShadedRay(
   ...
   drawCone( origin+direction*length, -direction,
      PI_F/8, direction.length()*arrowHeadLength, 5, false
   );
}
...
float widthOfStringToBeDrawn(
   ...
   if ( useTextureFont ) {
      switch ( fontHeightType ) {
         case DU_FONT_ASCENT :
            h = float(FontData::getCharAscent());
            break;
         case DU_FONT_ASCENT_PLUS_DESCENT :
         case DU_FONT_TOTAL_HEIGHT :
         default:
            h = float(FontData::getCharAscent() + FontData::getCharDescent());
            break;
      }
